...
export const XPostSkin = () => {
    ...
    return (
        <div className={`flex flex-col h-full font-sans ${isDarkMode ? 'bg-black text-[#e7e9ea]' : 'bg-white text-[#0f1419]'}`}>
            {/* Header / Top Bar Mock */}
            <div className={`flex items-center justify-between px-4 py-3 sticky top-0 z-10 backdrop-blur-md bg-opacity-80 border-b ${isDarkMode ? 'bg-black border-gray-800' : 'bg-white border-gray-100'}`}>
                ...
                <div className="w-6 h-6 flex items-center justify-center">
                    <svg viewBox="0 0 24 24" aria-hidden="true" className={`w-5 h-5 fill-current ${isDarkMode ? 'text-white' : 'text-black'}`}><g><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"></path></g></svg>
                </div>
            ...
            <div className={`px-4 pt-3 pb-2 border-b ${isDarkMode ? 'border-gray-800' : 'border-gray-100'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-1 overflow-hidden">
                                <span className={`font-bold text-[15px] truncate ${isDarkMode ? 'text-white' : 'text-[#0f1419]'}`}>
                                    {contact.name}
                                </span>
                                <CheckCircle2 className="w-[14px] h-[14px] text-blue-400 fill-current" />
                                <span className={`text-[15px] truncate ${isDarkMode ? 'text-[#71767b]' : 'text-[#536471]'}`}>
                                    @{contact.name.replace(/\s+/g, '').toLowerCase()} · 2h
                                </span>
                            </div>
                            <MoreHorizontal className={`w-4 h-4 shrink-0 ${isDarkMode ? 'text-[#71767b]' : 'text-[#536471]'}`} />
                        </div>
                        ...
                        {/* Image */}
                        {postConfig.image && (
                            <div className="mt-2 mb-3 rounded-2xl overflow-hidden border border-opacity-10 border-gray-500">
                        ...
                        {/* Action Bar */}
                        <div className={`flex items-center justify-between mt-1 max-w-[90%] ${isDarkMode ? 'text-[#71767b]' : 'text-[#536471]'}`}>
                            <div className="flex items-center gap-1 text-[13px] group cursor-pointer hover:text-blue-400">
                            ...
                            <div className="flex items-center gap-1 text-[13px] group cursor-pointer hover:text-green-500">
                                <div className="p-1.5 rounded-full group-hover:bg-green-500/10 transition-colors">
                                    <Repeat2 className="w-[18px] h-[18px]" strokeWidth={1.5} />
                                </div>
                                <span>{postConfig.shares}</span>
                            </div>
                            <div className="flex items-center gap-1 text-[13px] group cursor-pointer hover:text-pink-600">
                            ...
                            <div className="p-1.5 rounded-full hover:bg-blue-400/10 transition-colors cursor-pointer hover:text-blue-400">
             ...
             <div className="fixed bottom-6 right-6">
                <div className="w-12 h-12 rounded-full bg-blue-500 shadow-lg flex items-center justify-center text-white">
